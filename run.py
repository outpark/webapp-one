...
def kill_all_server_instances():
    port = str(ARGS.port)
    p1Full = subprocess.Popen(['lsof', '-i', ':' + port],
                             shell=False,
                             stdout=subprocess.PIPE)
    p1Pid = subprocess.Popen(['lsof', '-i', ':' + port, '-t'],
                             shell=False,
                             stdout=subprocess.PIPE)

    (data, err) = p1Full.communicate()
    index = 0
    lines = data.split('\n')[1:]
    for line in lines:
        if line.find("LISTEN") > 0:
            (pids, err) = p1Pid.communicate()
            pids = pids.split('\n')
            pid = pids[index]
            print "Killed running instance of appengine with pid: " + str(pid)
